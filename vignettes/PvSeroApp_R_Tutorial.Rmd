...
vignette: >
  %\VignetteIndexEntry{PvSeroApp in R Tutorial}
...
![](../man/figures/SeroTrackR_PvSeroApp.jpeg)
...
mfi_to_rau_output1      <- suppressMessages(MFItoRAU_Adj(sero_data, plate_list, qc_results, 10, project = NULL))
mfi_to_rau_output       <- suppressWarnings(MFItoRAU(sero_data, plate_list, qc_results, 10, project = NULL))
stdcurve_plot           <- suppressWarnings(plotStds(sero_data, "PNG", "experiment1", std_point = 10))
plateqc_plot            <- plotCounts(qc_results, "experiment1")
check_repeats_output    <- getRepeats(qc_results, plate_list)
blanks_plot             <- plotBlanks(sero_data, "experiment1")
model_plot              <- plotModel_Adj(mfi_to_rau_output1, sero_data)
