# Median Fluorescent Intensity (MFI) to Relative Antibody Units (RAU) conversion based on ETH standard
...
values into relative antibody units (RAU) written by Eamon Conway.

## Usage

``` r
MFItoRAU_ETH(sero_data, plate_list, counts_QC_output)
```
